...
| **[#145](https://github.com/ssalter21/tower-defense-game/issues/145)** — what the sweep's bot ranks by | Best coverage per gold, which is what "a deliberately dumb bot" obviously means | **Reversed by measurement. Cheapest thing that covers something new, then upgrade the oldest** | Measured against `content/map.txt`: three rangers cover all 47 route hexes for 120 gold, so *any* per-gold rule has bought the whole route by wave 2 and then does nothing for eight waves. It would have measured a game nobody plays — from the opposite direction to the one the ticket was opened to avoid |
