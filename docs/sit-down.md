...
those exact bytes. That is why this reads "drag to tick 1733, then back to tick
1691" rather than "hunt for the moment".
...
**Check the readout says the match ends on tick 5758 before you start.** That is
...
| `projectile-orphaned` | tick 2742 | shell 75 loses the creep it was aimed at, mid-flight |
| `first-leak` | tick 1733 | creep 25 reaches the exit |
| `last-creep-dies` | tick 5722 | creep 109, the last one, starts dying |

The match ends on tick 5758. Eighteen of forty creeps get through.
...
| 4 | **Scrub backwards from the mid-match landmark: drag to tick 1733, then drag slowly back to tick 1691** | Legs keep walking *forwards* — the view holds its own playback head and the animation bet is lost |
| 5 | Fast-forward: from tick 1733, press the speed button through to 8x | Walk cycle does not speed up. Same failure as 4, different symptom |
| 6 | Scrub back across the orphaned shell, which loses its target on tick 2742: drag to tick 2775, then back to tick 2715 | Projectile still flying, or a stuck death pose |
| 7 | Press To the end — tick 5758 — then drag the scrubber to tick 0 | A burst of effects all at once, or particles that never cleared |
| 8 | The projectile tower as it fires: nudge a tick at a time from tick 2709 to tick 2742 | Fires without playing its clip, or plays it without firing, or does not rotate to face its target |
| 9 | A creep at death: drag to tick 5685 and play at 1x through tick 5722 | Vanishes instantly instead of playing the death clip for the tick duration the simulation gave it |
