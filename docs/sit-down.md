...
those exact bytes. That is why this reads "drag to tick 1778, then back to tick
1736" rather than "hunt for the moment".
...
**Check the readout says the match ends on tick 5439 before you start.** That is
...
|---|---|---|
| `first-overtake` | tick 1096 | creep 22 draws ahead of creep 17 |
| `projectile-orphaned` | tick 890 | shell 28 loses the creep it was aimed at, mid-flight |
| `first-leak` | tick 1778 | creep 27 reaches the exit |
| `last-creep-dies` | tick 5403 | creep 109, the last one, starts dying |

The match ends on tick 5439. Eight of forty creeps get through — under the
...
| 4 | **Scrub backwards from the mid-match landmark: drag to tick 1778, then drag slowly back to tick 1736** | Legs keep walking *forwards* — the view holds its own playback head and the animation bet is lost |
| 5 | Fast-forward: from tick 1778, press the speed button through to 8x | Walk cycle does not speed up. Same failure as 4, different symptom |
| 6 | Scrub back across the orphaned shell, which loses its target on tick 890: drag to tick 923, then back to tick 863 | Projectile still flying, or a stuck death pose |
| 7 | Press To the end — tick 5439 — then drag the scrubber to tick 0 | A burst of effects all at once, or particles that never cleared |
| 8 | The projectile tower as it fires: nudge a tick at a time from tick 857 to tick 890 | Fires without playing its clip, or plays it without firing, or does not rotate to face its target |
| 9 | A creep at death: drag to tick 5366 and play at 1x through tick 5403 | Vanishes instantly instead of playing the death clip for the tick duration the simulation gave it |
