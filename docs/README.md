...
| V | [Tower & Creep Variance Levers](variance-levers-and-unit-schema.md) | What can a tower or a creep actually differ by, and what data structure holds all of it? | **One unit schema, two roles**, levers as components. Version the numbers apart from the vocabulary — and never silently skip an unknown lever. ⚠️ **Two Element TD 2 figures are wrong** — it ships **59** towers, not 57, and is **not** combinatorially complete (there is no five-element tower). Found by [Build depth in tower defense](research/build-depth-in-tower-defense.md); not yet corrected in the document itself. |
