...
| IV | [Art Direction & Asset Pack Strategy](art-direction-and-assets.md) | What does it look like, and what do we buy to get there fast? | Stylized 3D, not pixel art. Synty ships no animations — that inverts the obvious pick. ⚠️ **Its "KayKit Complete, $150" verdict no longer stands** — the current effort is free-tier only, $0. Owned by [#17](https://github.com/ssalter21/tower-defense-game/issues/17). The hex-kit mispricing was in **§5**, not §13, and is **now corrected in the document** by [#27](https://github.com/ssalter21/tower-defense-game/issues/27), along with §5's recommendation of a static-mesh pack for towers it requires to be animated. |
