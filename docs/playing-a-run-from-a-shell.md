...
| `send <type-id> <count>` | Fills the next wave slot, in the order the sends were typed — **and that order is the order the creeps walk out in**, so the sends are the arrangement rather than a list to be tidied. A creep some slot above it already sent is refused; nothing else about the order is. Sorting the slots would rewrite the decision on its author's behalf, and since #191 it would rewrite the fight as well |
