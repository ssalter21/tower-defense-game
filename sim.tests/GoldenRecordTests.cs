...
/// </para>
/// <para>
/// <b>Not every golden in that directory is a bundle, and the one that is not
/// is only read.</b> <c>content/golden/command-0.commands</c> is a command
/// stream, and the claim above -- evidence for a branch -- is the whole of what
/// it carries. Nothing below applies to it: it is not restaged, no table is
/// pinned beside it, and it is not held against the live
/// <c>content/run.commands</c>. Its own test says why.
/// </para>
/// <para>
...
public class GoldenRecordTests
{
    ...
    [Fact]
    public void The_frozen_command_stream_at_version_0_still_reads_back_its_commands()
    {
        // THIS IS EVIDENCE ABOUT A READ BRANCH AND ABOUT NOTHING ELSE. Reading
        // and replaying are separate gates -- reading needs a known format
        // version and canonical bytes, replaying needs the simulation version
        // and all three content hashes to match the run in front of it -- and
        // these bytes are taken through the first and never the second. No claim
        // is made that they play. A version-0 stream stores no placements, so a
        // run of it stands the opening board it is handed, and once that board
        // is empty the run reaches different numbers than the one that recorded
        // these decisions. That is expected, and it is why nothing here holds
        // this file against an outcome.
        //
        // The file is a copy of content/run.commands, taken while 0 is the
        // version the writer emits. It is a copy and not a second source: the
        // live file is regenerated by tools/run-headless-match.ps1 -Regenerate
        // and this one is not, so the two are equal today and are not kept
        // equal. What survives that is the only claim above -- a stream at
        // version 0 that this build can still read -- and it survives the format
        // bump that will stop version 0 being producible at all.
        string path = RepoLayout.GoldenCommandFile(0);

        Assert.True(
            File.Exists(path),
            "There is no command stream frozen at format version 0 at "
            + path
        ...
        // filled -- is what says the body decoded and not just the header.
        //
        // OBSERVED: doctor the last byte of content/golden/command-0.commands
        // from 0 to 2. It is the high half of the final slot's count, so the
        // slot reads back as 513 of type 13 rather than 1 and the comparison
        // goes red naming both decisions in full.
